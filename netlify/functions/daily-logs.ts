    ...
    switch (action) {
      case "FETCH_LOGS": {
        ...
        const { data, error } = await supabase
          .from("daily_logs")
          .select(
            "id, user_id, food_id, food_name, log_date, quantity_grams, calories_consumed, protein_consumed, carbs_consumed, fat_consumed, fiber_consumed, micros_snapshot",
          )
        ...
        console.log(
          `Successfully fetched logs for date range: ${startDate} to ${endDate}`,
        );
